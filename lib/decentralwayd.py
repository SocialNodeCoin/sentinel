"""
decentralwayd JSONRPC interface
"""
import sys
import os
...
class decentralwaydaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some decentralwayd methods
    ...
    @classmethod
    def from_decentralway_conf(self, decentralway_dot_conf):
        from decentralway_config import decentralwayConfig
        config_text = decentralwayConfig.slurp_config_file(decentralway_dot_conf)
        creds = decentralwayConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = decentralwaylib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = decentralwaylib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = decentralwaylib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_decentralwayd_proto_version_with_sentinel_ping)
