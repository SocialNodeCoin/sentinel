...
def is_valid_allgamescoin_address(address, network='mainnet'):
    ...
    allgamescoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid AllGamesCoin address
        return False

    if (address_version != allgamescoin_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.allgamescoind_serialise())
...
def SHIM_serialise_for_allgamescoind(sentinel_hex):
    from models import GOVOBJ_TYPE_STRINGS

    # unpack
    obj = deserialise(sentinel_hex)

    # shim for allgamescoind
    govtype_string = GOVOBJ_TYPE_STRINGS[obj['type']]

    # superblock => "trigger" in allgamescoind
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # allgamescoind expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    allgamescoind_hex = serialise(obj)
    return allgamescoind_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('allgamescoin.conf')
