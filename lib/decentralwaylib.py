...
def is_valid_decentralway_address(address, network='mainnet'):
    ...
    decentralway_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid decentralway address
        return False

    if (address_version != decentralway_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.decentralwayd_serialise())
...
def SHIM_serialise_for_decentralwayd(sentinel_hex):
    from models import GOVOBJ_TYPE_STRINGS

    # unpack
    obj = deserialise(sentinel_hex)

    # shim for decentralwayd
    govtype_string = GOVOBJ_TYPE_STRINGS[obj['type']]

    # superblock => "trigger" in decentralwayd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # decentralwayd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    decentralwayd_hex = serialise(obj)
    return decentralwayd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('decentralway.conf')
