"""
allgamescoind JSONRPC interface
"""
import sys
import os
...
class AllGamesCoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some allgamescoind methods
    ...
    @classmethod
    def from_allgamescoin_conf(self, allgamescoin_dot_conf):
        from allgamescoin_config import AllGamesCoinConfig
        config_text = AllGamesCoinConfig.slurp_config_file(allgamescoin_dot_conf)
        creds = AllGamesCoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = allgamescoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = allgamescoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = allgamescoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_allgamescoind_proto_version_with_sentinel_ping)
